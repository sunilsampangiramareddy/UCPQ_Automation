...
import pageObjects_StorageGrid.HomePageStorageGrid;
...
public class TC_StorageGridConfig extends BaseClass{
	...
	public void createStorageGridConfig (String userName, String accountName, String opportunityType, String opportunityName, String primaryContact, String salesPlay, String salesType, String installedBaseType, String currency, String channel, String hyperscaler, String pathway, String partnerSalesModel, String endCustomerUsage, String reseller, String resellerSalesRep, String resellerSE, String product, String salesPrice, String subProduct, String modelStorageNodes, String driveTypeStorageNodes, String qtyStorageNodes, String expectedLabel) throws InterruptedException, IOException
	{
		...
		HomePageStorageGrid hpsg=new HomePageStorageGrid(driver);
		...
		logger.info("Started TC_StorageGridConfig Test Execution");
...
//--------------------Configure Storage Grid product-------------------------------------------------------------
			hpc.clickProdctsTab();
			...
			hpsg.clickStorageGridProdcut();
			logger.info("Clicked on storage grid product");		
			ExtentReport.logStep(driver, "Clicked on storage grid product");
			
			hpsg.clickConfigureconfigureStorageGRIDApplianceAndSoftware(subProduct);
			logger.info("click on subproduct "+subProduct);		
			ExtentReport.logStep(driver, "click on subproduct-"+subProduct);
			
			hpsg.selectModelStorageNodes(modelStorageNodes);
			logger.info("Storage nodes model selected as "+modelStorageNodes);	
			ExtentReport.logStep(driver, "Storage nodes model selected-"+modelStorageNodes);
			
			hpsg.selectDriveTypeStorageNodes(driveTypeStorageNodes);
			logger.info("Storage nodes drive type selected as "+driveTypeStorageNodes);	
			ExtentReport.logStep(driver, "Storage nodes drive type selected-"+driveTypeStorageNodes);
			
			hpsg.enterQtyStorageNodes(qtyStorageNodes);
			logger.info("Storage nodes quantity entered "+qtyStorageNodes);
			ExtentReport.logStep(driver, "Storage nodes quantity entered-"+qtyStorageNodes);
			
			bp.captureScreenshot(driver);
			logger.info("Captured screen shot of storage grid product config");	
			ExtentReport.logStep(driver, "Captured screen shot of storage grid product config");
			
			hpsg.clickAddToQuote();
			logger.info("Clicked on add to quote");	
			ExtentReport.logStep(driver, "Clicked on add to quote");
...
//--------------------Validate Product table details in Products page-------------------------------------------------------------
			...
			ExtentReport.logStep(driver, "Reading prodcut column data from products table");
